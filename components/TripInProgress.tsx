...
const TripInProgress: React.FC<TripInProgressProps> = ({ trip, onEndTrip }) => {
  ...
  useEffect(() => {
    ...
    // This check prevents crashing in environments where process.env is not defined.
  ...
  return (
    <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-gray-100 mt-8">
      ...
      <div className="bg-gray-50 p-4 rounded-lg border mb-6">
          <h3 className="font-bold text-lg mb-4">Detalhes da Viagem</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
              <div className="space-y-4">
                  <div className="flex items-start">
                      <LocationMarkerIcon className="h-5 w-5 text-gray-500 mr-3 mt-0.5 flex-shrink-0" />
                      <div>
                          <p className="font-semibold text-gray-800 text-sm">Origem</p>
                          <p className="text-gray-600 text-sm">{trip.origin}</p>
                      </div>
                  </div>
                  <div className="flex items-start">
                      <LocationMarkerIcon className="h-5 w-5 text-red-500 mr-3 mt-0.5 flex-shrink-0" />
                      <div>
                          <p className="font-semibold text-gray-800 text-sm">Destino</p>
                   ...
                   <div className="flex items-start">
                      ...
                      <div>
                          <p className="font-semibold text-gray-800 text-sm">Motorista</p>
                          <p className="text-gray-600 text-sm">{trip.driver.name}</p>
                      </div>
                  </div>
              </div>
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 text-center h-full flex flex-col justify-center">
                  <h3 className="font-bold text-sm mb-2 flex items-center justify-center"><ShieldCheckIcon className="h-5 w-5 mr-2 text-blue-500" /> Código de Embarque</h3>
                  <p className="text-3xl font-mono tracking-widest text-blue-800 bg-white p-2 rounded-lg">{trip.pickupCode}</p>
                  <p className="text-xs text-gray-500 mt-2">Mostre ao motorista.</p>
              </div>
          </div>
      </div>
